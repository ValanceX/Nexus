...
| 9 | `analyze`: rebuilds profile from raw context | "analyze touches the input exactly as build does, for v0.4-shaped and data-flow contexts" | semantic-build.test.ts (1) |
...
| 12 | `build`: profile checks moved before the value loop | issue-order test; two D6 rejection-order tests; the v0.4 differential test | semantic-build.test.ts, semantic.test.ts (4 across 3 files) |
...
| 15 | `locationOf`: no `copySpan` (aliases input) | "output spans are fresh { source, start, end }, without forged extras"; "analyze touches the input exactly as build does..." | semantic-build.test.ts (2) |
