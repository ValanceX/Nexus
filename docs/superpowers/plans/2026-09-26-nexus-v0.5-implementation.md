...
**Spec:** `docs/superpowers/specs/2026-09-26-nexus-v0.5-outline.md` (Revision 2, approved 2026-09-26, with D28 approved). The outline wins wherever the two disagree.
...
- **v0.4 tests.** Existing test cases change only where D28 allows (B5 read allowlist, B6 exports pin, B7 rejection-reason pin), plus the export-count pin covered by open item O1 below. New tests go in new files, or in new `describe` blocks appended at the end of an existing file.
...
**O1. A fourth v0.4 pin.** The outline's D28 lists three amendments, but `tests/semantic.test.ts:673` (the N4 docs-parity test) also pins the export count: `expect(exported.size).toBe(20)`. v0.5 adds 15 exports (`build` and 14 types), so the test must change from `20` to `35`. The change is of the same widening-only kind as D28, and the rest of that test (documented names must equal exported names, and every interface field must be documented) stays as it is. **This plan includes the change in Task 2 and needs your approval for it.** If it isn't approved, Task 2 cannot pass.
...
- [ ] **Step 6: Amend the three pinned v0.4 tests (B6, B7, O1)**

This is the complete set of changes to `tests/semantic.test.ts`:
...
- Line 673: `expect(exported.size).toBe(20);` becomes `expect(exported.size).toBe(35);`. **This one depends on O1 being approved.**
...
Expected: all PASS. The differential gate (Task 1), the 134 unchanged v0.4 tests and the new tests are all green.
...
- the v0.4 test amendments (B5, B6, B7 and O1), listed verbatim;
...
- [ ] **Step 3: Confirm the v0.4 tests changed only as allowed.** Run `git diff v0.4.0 -- tests/semantic.test.ts tests/semantic-isolation.test.ts tests/semantic-types.test.ts tests/architecture.test.ts`. Expected:
  - `tests/semantic.test.ts`: the three pin amendments (lines 43, 65, 673) only;
...
| D28 (+ O1): test amendments | Task 2 Step 6; Task 4 Step 4; Task 7 Step 3 |
