...
| Deliberate-break table (outline §10) | Task 7 rows 1–14 cover all 13 outline rows. Rows 15–16 are extra. |
...
Task 7 ran each break below against `src/semantic/index.ts` at `b78fee4`, one at a time, then reverted with `git checkout -- src/` (confirmed clean via `git diff --stat src/` before the next row). All 16 breaks were caught by the existing suite; none survived, so no test was added and `src/` ends unchanged from `b78fee4`.

| # | Break | Failing tests (brief) | Files affected |
|---|---|---|---|
| 1 | `flowOf`: undefined → complete-empty | "absent inputs and outputs are Unknown"; 3 openness tests (closed-empty vs open-empty; openedBy; partial-empty) | semantic-build.test.ts (5) |
| 2 | `flowOf`: partial-empty → Unknown | "a partial fact with no references stays Declared partial-empty..."; "partial-empty opens its side exactly as unknown does" | semantic-build.test.ts (2) |
| 3 | `relationshipSet`: drop partial from openedBy | "openedBy is exactly the unknown and partial operations..."; "partial-empty opens its side..." | semantic-build.test.ts (2) |
| 4 | `relationshipSet`: drop Unknown from openedBy | "closed-empty and open-empty are different tags"; openedBy test; partial-empty test; "each side's openness ignores the other side" | semantic-build.test.ts (4) |
| 5 | `relationshipSet`: openedBy for complete facts too | "a value may share an identity with an operation and a capability"; "names create nothing"; closed-empty test; openedBy test; partial-empty test; "each side's openness ignores the other side"; "an operation appears once in members..."; may-flow cart test | semantic-build.test.ts (8) |
| 6 | `build`: add `edges: []` | "stores no edge list: Built, its operations and its values have exactly their documented fields" | semantic-build.test.ts (1) |
| 7 | `relationshipSet`: member from matching `Name` | "names create nothing (I14): a value named like an operation gains no relationship" | semantic-build.test.ts (1) |
| 8 | `build`: synthetic `{ id: "*" }` value on Unknown | "an absent values list builds exactly like an empty one"; "creates no synthetic or wildcard value for unknown facts" | semantic-build.test.ts (2) |
| 9 | `analyze`: rebuilds profile from raw context | "analyze touches the input exactly as build does, for v0.4-shaped and data-flow contexts" | semantic-compatibility.test.ts (1) |
| 10 | `analyzeOperation`: `known` empty when outputs Unknown | 11 compatibility tests (supported/opaque/incompatible cases) plus the v0.4 differential test and the data-flow-never-affects-compatibility test | semantic.test.ts, semantic-compatibility.test.ts (13) |
| 11 | `flowOf`: drop `unresolved-reference` push | "an unresolved reference is rejected..."; "a reference to an operation or capability identity is unresolved..."; "a reference to a value rejected for a missing identity is unresolved"; issue-order test | semantic-build.test.ts (4) |
| 12 | `build`: profile checks moved before the value loop | issue-order test; two D6 rejection-order tests; the v0.4 differential test | semantic-build.test.ts, semantic.test.ts, semantic-compatibility.test.ts (4 across 3 files) |
| 13 | `build`: `missing-identity` at `["values"]` when absent | Nearly every v0.4-shaped test that omits `values` — data-flow-states, values/references, D6 malformed-context cases, outcome envelope, all compatibility/diagnostic/purity/independence suites, plus the differential and data-flow tests | semantic-build.test.ts, semantic.test.ts (~90 across 4 files) |
| 14 | `flowOf`: reference pushed only on first occurrence | "keeps value, operation and every reference occurrence's provenance distinct" | semantic-build.test.ts (1) |
| 15 | `locationOf`: no `copySpan` (aliases input) | "output spans are fresh { source, start, end }, without forged extras"; "analyze touches the input exactly as build does..." | semantic-build.test.ts, semantic-compatibility.test.ts (2) |
| 16 | Import `Effect` from `"effect"` and use it | "is one file with no imports" (architecture.test.ts); mock-hoisting error in semantic-no-primitives.test.ts (a suite failure, not a named test, but the run fails) | architecture.test.ts, semantic-no-primitives.test.ts (1 test + 1 suite error) |

Step 2 (clean tree): confirmed after every row; `git diff --stat src/` was empty before each subsequent break.

Step 3 (v0.4 test diff, `git diff v0.4.0 -- tests/semantic.test.ts tests/semantic-isolation.test.ts tests/semantic-types.test.ts tests/architecture.test.ts`): matched the brief exactly, nothing extra —
- `tests/semantic.test.ts`: only the B7 title/union change (`RejectionReason` gains `unresolved-reference`), the B6 title/export-list change (`["analyze"]` → `["analyze", "build"]`), and the O1 count change (`toBe(20)` → `toBe(35)`).
- `tests/semantic-isolation.test.ts`: only the `modeled` allowlist gaining `values`, `inputs`, `outputs`, `references`, `value`, and its comment.
- `tests/semantic-types.test.ts`: only an appended `describe` block at the end of the file.
- `tests/architecture.test.ts`: no diff.

Step 4 (full verification, final run): `npm run typecheck && npm test && npm run build`, all green.
- Test files: 20 (17 pre-existing + 3 new semantic files: `semantic-build.test.ts`, `semantic-compatibility.test.ts`, `semantic-no-primitives.test.ts`), matching the expected 17 + 3 = 20.
- Tests: 287 passed, 0 failed.
- `src/` unchanged from `b78fee4` (no surviving breaks, so no test additions were needed).
